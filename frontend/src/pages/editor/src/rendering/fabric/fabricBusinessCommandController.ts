...
const CLICK_HIT_THRESHOLD = 6;
const HOVER_HIT_THRESHOLD = 8;
...
export function bindFabricBusinessCommandEvents({
  ...
  let cachedCandidateDocument: DocumentState | null = null;
  let cachedCandidateState: ActiveBusinessCommandState | null = null;
  let cachedCandidateNodes: EditorNode[] = [];
  let cachedPreviewLabelDocument: DocumentState | null = null;
  let cachedPreviewLabelState: ActiveBusinessCommandState | null = null;
  let cachedPreviewLabelNodeIds = new Set<NodeId>();
  let cachedLineHighlightState: ActiveBusinessCommandState | null = null;
  let cachedLineHighlightMap = new Map<string, LineHighlightInfo>();
  const emptyPreviewLabelNodeIds = new Set<NodeId>();

  const getCandidateNodes = (state: ActiveBusinessCommandState) => {
    const document = getDocument();
    if (
      cachedCandidateDocument === document &&
    ...
    cachedCandidateDocument = document;
    cachedCandidateState = state;
    cachedCandidateNodes = resolveCandidateNodes(document, state);
    return cachedCandidateNodes;
  };

  const getPreviewLabelNodeIds = (state: ActiveBusinessCommandState) => {
    if (!isSelectionToolActive(activeToolId)) {
      return emptyPreviewLabelNodeIds;
    }

    const document = getDocument();
    if (
      cachedPreviewLabelDocument === document &&
      cachedPreviewLabelState === state
    ) {
      return cachedPreviewLabelNodeIds;
    }

    cachedPreviewLabelDocument = document;
    cachedPreviewLabelState = state;
    cachedPreviewLabelNodeIds = new Set(
      buildPreviewLabelMap(document, state).keys(),
    );
    return cachedPreviewLabelNodeIds;
  };

  const getLineHighlightMap = (state: ActiveBusinessCommandState) => {
    if (cachedLineHighlightState === state) {
      return cachedLineHighlightMap;
    }

    cachedLineHighlightState = state;
    cachedLineHighlightMap = resolveLineHighlightMap(state);
    return cachedLineHighlightMap;
  };
  ...
  const refresh = () => {
    ...
    const previewLabelNodeIds = getPreviewLabelNodeIds(businessCommand);
    ...
    applyVisualState(
      canvas,
      renderDocument,
      getLineHighlightMap(businessCommand),
      pendingBrushHitMap,
  ...
  const handleMouseMove = (evt: { scenePoint?: CanvasPoint }) => {
    ...
    const candidateNodes = getCandidateNodes(businessCommand);
    if (!activePointer) {
      const nextHoveredLineId =
        findNearbyLines(point, candidateNodes, HOVER_HIT_THRESHOLD)[0]
          ?.nodeId ?? null;
      if (hoveredLineId === nextHoveredLineId) {
        return;
      }
      hoveredLineId = nextHoveredLineId;
      refresh();
      return;
    }
