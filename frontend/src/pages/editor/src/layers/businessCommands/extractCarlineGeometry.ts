...
const extractableLineGeometryCache = new WeakMap<
  EditorNode,
...
export function resolveExtractableLineGeometry(
  ...
  const cachedGeometry = extractableLineGeometryCache.get(node);
  ...
  if (node.fabricObject.type === "line") {
    ...
    const geometry: ResolvedLineGeometry = {
      nodeId: node.id,
      sourceType: "line",
      segments: [
        {
    ...
    extractableLineGeometryCache.set(node, geometry);
    return geometry;
  }
