...
function createCarlineLabelNodeId(areaName: string, lineNodeId: NodeId): NodeId {
  return `${PREVIEW_LABEL_NODE_PREFIX}${areaName}/${lineNodeId}`;
}
...
function buildPreviewLabelSpecs(
  ...
  return areaDraft.selectedLines.map((selectedLine, index) => ({
    id: createCarlineLabelNodeId(areaDraft.areaName, selectedLine.nodeId),
...
export function applyExtractCarlineSession(
  ...
  for (const areaDraft of getPreviewAreas(session)) {
    for (const selectedLine of areaDraft.selectedLines) {
      globalOrder++;
      const 标注NodeId = createAnnotationNodeIdMap();
      标注NodeId.车线编号 = createCarlineLabelNodeId(
        areaDraft.areaName,
        selectedLine.nodeId,
      );
      carlines.push({
        ...
        编号: globalOrder,
        区域: areaDraft.areaName,
        车线编号: `${areaDraft.areaName}${String(globalOrder).padStart(2, "0")}`,
        尺数: areaDraft.carlineLength,
        档位: "",
        DML: "D",
        是双数: false,
  ...
  nextDocument.domain = {
    ...
    车线: carlines,
    标注样式: {
      ...nextDocument.domain.标注样式,
      车线编号: {
        ...nextDocument.domain.标注样式.车线编号,
        字体:
          nextDocument.domain.标注样式.车线编号?.字体 ??
          "system-ui, -apple-system, Segoe UI, Roboto, sans-serif",
        字号: session.currentDraft.labelFontSize,
        字色: session.currentDraft.labelColor,
      },
    },
  };
