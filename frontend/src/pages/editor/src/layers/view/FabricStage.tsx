...
import { bindFabricBusinessCommandEvents } from "../../rendering/fabric/fabricBusinessCommandController";
...
function buildExportSvg(
  ...
  viewState: ViewState,
  includeAnnotationBackgrounds = true,
) {
...
>(function FabricStage(
  ...
  useEffect(() => {
    ...
    canvas.set({
      backgroundColor:
        renderDocument.canvas.backgroundColor &&
        renderDocument.canvas.backgroundColor.trim() &&
        renderDocument.canvas.backgroundColor !== "transparent"
          ? renderDocument.canvas.backgroundColor
          : undefined,
    });
    canvas.setViewportTransform(
    ...
    // (e.g. a pending RAF from canvas.add() fires before applyAnnotationBackgroundToTextNode
    // positions the background, showing a default-sized white rect)
    canvas.renderAll();
  ...
  useEffect(() => {
    ...
    return bindFabricBusinessCommandEvents({
      ...
      updateBusinessCommand: (updater) =>
        editor.edit.updateBusinessCommand(updater),
    });
  }, [activeToolId, editor]);
