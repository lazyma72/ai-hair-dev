...
export type EditorShellProps = {
  ...
  onDocumentChange?: (payload: { document: DocumentState; svg: string }) => void;
};
...
export function EditorShell({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const svg = stageRef.current?.exportPersistedSvg() ?? "";
      onDocumentChange({ document: editor.data.getState(), svg });
    }, 180);
