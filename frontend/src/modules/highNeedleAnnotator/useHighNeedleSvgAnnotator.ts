...
type ManualDmlOverride = DmlValue | "";
...
function normalizeRegionLineSort(value: unknown): number | null {
...
function normalize区域线条(
  ...
  return items
    ...
    .map((item, index) => ({
      item,
      sort: normalizeRegionLineSort(item.sort) ?? index + 1,
      index,
    }))
...
function normalize高针图值(value: 高针图): 高针图 {
  return {
    ...
    底图: {
      ...value.底图,
      区域名: value.底图.区域名 ?? [],
      区域线条: normalize区域线条(value.底图.区域线条 ?? []),
...
function buildRegionSegmentsFromLineIds(
  orderedRegionLinesByName: Map<string, string[]>,
  regionOrder: string[],
...
function mergeManualDmlAssignments(
  base: Map<string, DmlValue>,
  manual: Record<string, ManualDmlOverride>,
): Map<string, DmlValue> {
  const merged = new Map<string, DmlValue>(base);
  Object.entries(manual).forEach(([id, value]) => {
    const normalized = String(value ?? "").trim() as ManualDmlOverride;
    if (normalized === "") {
      merged.delete(id);
      return;
    }
...
export default function useHighNeedleSvgAnnotator({
  ...
  const manualDmlOverridesRef = useRef<Record<string, ManualDmlOverride>>({});
  ...
  const preferredRegionPosByLineId = useMemo(() => {
    ...
    return map;
  }, [actualRegionTextNodeIdByLineId, value.底图.svg]);

  const preferredLevelPosByLineId = useMemo(() => {
    const map = new Map<string, SvgPoint>();
  ...
  useEffect(() => {
    ...
    let changed = false;
    actualRegionTextNodeIdByLineId.forEach((_, lineId) => {
      if (!draftMap.has(lineId)) return;
      if (!preferredRegionPosByLineId.has(lineId)) return;
      draftMap.delete(lineId);
      changed = true;
    });

    if (changed) {
      draftMarkerPosByLineIdRef.current = new Map(draftMap);
    }
  }, [actualRegionTextNodeIdByLineId, preferredRegionPosByLineId]);

  useEffect(() => {
    if (pendingLevelMarkerPosByLineId.size === 0) return;

    let changed = false;
    const next = new Map(pendingLevelMarkerPosByLineId);
    pendingLevelMarkerPosByLineId.forEach((_, lineId) => {
      if (!preferredLevelPosByLineId.has(lineId)) return;
      next.delete(lineId);
      changed = true;
    });
  ...
  const regionLineItems = useMemo<RegionLineItem[]>(() => {
    return value.底图.区域线条
      .map((item, index) => {
        const lineId = String(item.lineNodeId ?? "").trim();
        if (!lineId) return null;
        return {
          lineId,
          regionName: String(item.区域名 ?? "").trim(),
          sort: normalizeRegionLineSort(item.sort) ?? index + 1,
        };
      })
      .filter((item): item is RegionLineItem => item !== null)
  ...
  const regionNoById = useMemo(() => {
    ...
    value.底图.区域线条.forEach((d, idx) => {
      const lineId = String(d.lineNodeId ?? "").trim();
      if (!lineId) return;
      map.set(lineId, normalizeRegionLineSort(d.sort) ?? idx + 1);
    });
  ...
  const availableForStep = useMemo(() => {
    ...
    if (step === "档位") {
      const archived = new Set(regionLineIdsInOrder);
  ...
  function patchActiveRangeRuleCommands(
    commands: DML规则命令[],
    nextLineIds: string[],
  ) {
    let changed = false;
    ...
    const nextCommands = commands.map((item) => {
      ...
      if (item.type !== "区域百分比" && item.type !== "按档位标记") {
        return item;
      }
  ...
  function getDmlBrushPreview(
    lineIds: string[],
    mode: "add" | "remove" | "toggle",
  ): DmlBrushPreview {
    if (step !== "DML" || !activeDmlRuleId || !activeDmlRuleType) {
      return {
        previewByLineId: new Map(),
        affectedLineIds: [],
      };
    }

    const normalizedLineIds = uniquePreserveOrder(
      lineIds
        .map((lineId) => String(lineId ?? "").trim())
        .filter((lineId) => availableForStep.has(lineId)),
    );
    ...
    if (activeDmlRuleType === "特殊标记") {
      ...
      const nextManual = { ...manualDmlOverridesRef.current };
      normalizedLineIds.forEach((lineId) => {
        const existedSameValue = nextManual[lineId] === normalizedValue;
        if (mode === "remove" || (mode === "toggle" && existedSameValue)) {
          delete nextManual[lineId];
          return;
        }
      ...
      normalizedLineIds.forEach((lineId) => {
        ...
        if (nextValue) {
          previewByLineId.set(lineId, nextValue);
        }
      });
      return {
        previewByLineId,
        affectedLineIds: normalizedLineIds,
      };
    }

    const nextLineIdSet = new Set(activeDmlRuleLineIds);
    normalizedLineIds.forEach((lineId) => {
      const existed = nextLineIdSet.has(lineId);
      if (mode === "remove") {
        nextLineIdSet.delete(lineId);
        return;
      }
      if (mode === "toggle" && existed) {
        nextLineIdSet.delete(lineId);
        return;
      }
      nextLineIdSet.add(lineId);
    });

    const nextLineIds = Array.from(nextLineIdSet);
    const affectedLineIds = uniquePreserveOrder([
      ...activeDmlRuleLineIds,
      ...nextLineIds,
    ]);
    const { nextCommands } = patchActiveRangeRuleCommands(
      value.自定义数据.DML规则命令列表,
      nextLineIds,
    );
  ...
  function handleLineDmlCycleOverride(lineId: string, markerPos?: SvgPoint) {
    ...
    if (next === "") {
      manualDmlOverridesRef.current[lineId] = "";
      dmlMarkerPosByLineIdRef.current.delete(lineId);
    } else {
  ...
  function applyDmlBrushSelection(
    lineIds: string[],
    mode: "add" | "remove" | "toggle",
    markerPosByLineId?: Map<string, SvgPoint>,
  ) {
    if (step !== "DML") return;
    if (!activeDmlRuleId || !activeDmlRuleType) return;
    ...
    if (activeDmlRuleType === "特殊标记") {
      ...
      normalizedLineIds.forEach((lineId) => {
        const existedSameValue =
          manualDmlOverridesRef.current[lineId] === normalizedValue;
        ...
        manualDmlOverridesRef.current[lineId] = normalizedValue;
        const markerPos = markerPosByLineId?.get(lineId);
        if (markerPos) {
          dmlMarkerPosByLineIdRef.current.set(lineId, markerPos);
        }
      });
  ...
  function ensureRegionMarkerTextNode(lineNodeId: string, pos: SvgPoint) {
    ...
    setValue((cur) => {
      ...
      if (existingTextNodeId) {
        // Ensure persisted marker text content stays correct (avoid showing raw ids like `region_text_*`).
        let nextSvg = updateSvgTextNode(
          cur.底图.svg,
          existingTextNodeId,
          regionNo,
        );
        nextSvg = setSvgTextNodeStyle(
          nextSvg,
          existingTextNodeId,
          getMarkerTextFontStyle("region"),
        );
        nextSvg = setSvgTextNodePosition(nextSvg, existingTextNodeId, pos);
        if (nextSvg === cur.底图.svg) return cur;
        return {
          ...cur,
          底图: {
            ...cur.底图,
  ...
  function finishRegion(options?: { gotoNextStage?: boolean }) {
    ...
    setValue((v) => {
      ...
      const newLines = selected.map((lineId, i) => {
        ...
        const sort = startNoInSetter + i + 1;
        const regionNo = String(sort);
        const result = upsertMarkerTextNode(nextSvg, {
          textNodeId: "",
          createNodeId: allocLocalId("region_text"),
  ...
  function finishLevel() {
    ...
    setPendingLevelMarkerPosByLineId((prev) => {
      const next = new Map(prev);
      selected.forEach((lineNodeId) => {
        const pos = draftMarkerPosByLineIdRef.current.get(lineNodeId);
        if (!pos) return;
        next.set(lineNodeId, pos);
      });
      return next;
    });
    setDraftSelected([]);
  ...
  function goNextStep() {
    ...
    if (step === "自定义文本") {
      ...
      // Exclude DML marker text nodes from the final SVG output.
      // Double marker text should be kept as-is to preserve the user's
      // manually confirmed position.
      const dmlTextIds = new Set<string>();
      actualDmlTextNodeIdByLineId.forEach((id) => dmlTextIds.add(id));

      const keepTextNodeIds = uniquePreserveOrder([
        ...Object.values(value.底图.文本节点).map((d) => d.textNodeId),
        ...Array.from(markerTextIdSet).filter((id) => !dmlTextIds.has(id)),
  ...
  function updateTextNodeFontSize(key: string, fontSize: number) {
    const nextFontSize = Math.max(4, Math.min(200, Math.round(fontSize)));
