  ...
  return (
    <div className="mt-4 space-y-3">
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <div className="mb-1 text-[11px] font-medium text-slate-600">预置区域</div>
          <select
            ...
            onChange={(e) => {
              ...
              if (!preset) return;
              setRegionDraft({ name: preset.name, lineLength: preset.lineLength });
            }}
          >
        ...
        <div>
          <div className="mb-1 text-[11px] font-medium text-slate-600">线条长度</div>
          <input
            type="number"
            className="w-full rounded border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-slate-300"
      ...
      <div>
        <div className="mb-1 text-[11px] font-medium text-slate-600">
          区域名
          {regionPresetValue === CUSTOM_REGION_PRESET_VALUE ? "（自定义）" : "（预置，不可编辑）"}
        </div>
        <input
          ...
          onChange={(e) => setRegionDraft((d: any) => ({ ...d, name: e.target.value }))}
        />
      </div>

      <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
        ⚠️ 按顺序标注：区域顺序会影响后续 DML 排列顺序；并且区域序号会跨区域累增。
      </div>
        ...
        <button
          ...
          onClick={() => {
            if (draftSelected.length > 0) {
              finishRegion({ gotoNextStage: true });
              return;
            }
            goNextStep();
          }}
      ...
      <div className="text-[11px] text-slate-500">
        操作：按住鼠标左键拖动经过线条，可连续勾选/取消（一次拖动内同一条线只会切换一次）。当前已选 {draftSelected.length}
